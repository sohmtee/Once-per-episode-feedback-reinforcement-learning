...
policy_opt =[0,3,3,0,3,1,0,0,1,0,0,2,1,2,3,0] 
gamma = 0.95
error = 0.01
...
def approximate_return(T,w,sigma,t):
    ...
    d2 =[]
    for s in T:
       d1.append(abs(s[0]-2) + abs(s[1]-2))
       d2.append(abs(s[0]-3) + abs(s[1]-3))
...
def calculate_reward(T,Kval):
    ...
    phie = -dist/5+1
    kk=Kval
    r = np.linspace(0,1,kk)
    
    e = math.e
    mu = 1/(1+e**(-3*phie))
    r1 = 5*(1-np.absolute(r-mu))
    dist = np.exp(r1)/np.sum(np.exp(r1)) 
    
    y =  np.random.choice(range(kk), size=1,  p=dist)      
    return y[0]
...
def objective_function(x, f, y,Kval):
    ...
    tmp = np.tile(tmp,(bss,1))
    tmp= np.transpose(tmp)
    r1 = 5*(1-np.absolute(r-tmp))
    dist =np.transpose( np.exp(r1))/np.sum(np.exp(r1),1) 
    dist = np.transpose(dist)
    res = x**2/2
    for jj in range(kk):
      res = res + -1*np.dot(y==jj,np.log(dist[:,jj]))
    
    return res
    
def solve_optim(feedback,feature_vec,Kval):
    # Initial guess
    initial_guess = [1]
    ...
    result = minimize(objective_function, initial_guess, args=(constant1, constant2,Kval), method='BFGS')
    
    # Print the result
    return result.x


def K_UCBVI(Final,Step,Rept,Kval):
        import numpy as np           
        import math          
        final_out_Reward =[]
        final_out_Err = []
        ...
        for enddd in range(Step,Final,Step):
            ...
            for numm in range(Rept):
                    ...
                    for i in range(enddd):
                        ...
                        feedback.append(calculate_reward(T,Kval))
                        feature_vec.append(apply_phie(T))
                        sigma = (2.71828)**3+np.dot(feature_vec,feature_vec)
                    
                    w_est = solve_optim(feedback,feature_vec,Kval)
                    #print(w_est)
